...
function Main({ filters }) {
    ...
    useEffect(() => {
        ...
        if (filters.customer) {
            result = result.filter(x => String(x.customer) === String(filters.customer));
        }

        if (filters.executor) {
            result = result.filter(x => x.executor?.toLowerCase().includes(filters.executor.toLowerCase()));
        }

        if (filters.department) {
            result = result.filter(x => String(x.department) === String(filters.department));
        }

        if (filters.status) {
            result = result.filter(x => String(x.states) === String(filters.status));
        }
    ...
    const deleteDocks = () => {
        ...
        axios.delete(`http://localhost:8080/delete?id=${selectedId}`)
            .then(() => {
                setFilteredDocks(prev => prev.filter(d => d.idContract !== selectedId));
                setSelId(null);
            })
            .catch(err => {
    ...
    return (
        <div className="Main">
            <div className="RedactBar">
                <button className="crateDock" onClick={navigate}>Добавить</button>
